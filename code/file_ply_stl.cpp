					...
					{// head
                        ...
                        for (i=0;i<Num_vertices;i++) {
                            for (j=0;j<3;j++) {
                                ...
                                if (Next_token==NUMBER) {
                                    ...
                                    error("no number for coordinate");
							}
						}
                        // faces data
                        for (i=0;i<Num_faces;i++) {
                            ...
                            if (Next_token==NUMBER) {
                                ...
                                error("no number of vertex indices");
                        ...
                        error("no format number");
                } else
                    error("no ascii format");
            } else
                error("no format word");
        } else
            error("this is not a ply file");
        return(0);
...
int _file_ply::is_number(char *Aux_token) {
    ...
    for (int i=0;i<strlen(Aux_token);i++) {
        if ((p1=index("0123456789.+-eE",Aux_token[i]))==NULL) return(-1);
	}
